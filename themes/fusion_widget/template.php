<?php
function fusion_widget_preprocess_node(&$vars, $hook) {
  if ($vars['teaser']) {
    $vars['template_files'][] = "node-{$vars['node']->type}-teaser";
  }

  $function = "fusion_widget_preprocess_node_{$vars['node']->type}";
  if (function_exists($function)) {
    $function($vars);
  }
}

function fusion_widget_preprocess_node_product(&$vars) {
  $node = $vars['node'];
  if (!$vars['teaser']) {
    drupal_add_css(drupal_get_path('theme', 'fusion_widget') . '/css/node.css');
    drupal_add_js(drupal_get_path('theme', 'fusion_widget') . '/js/products.js');
  }

  $vars['phaseout'] = (array_key_exists(95, $node->taxonomy))?true:false;
}

function fusion_widget_nice_menu_build($menu, $depth = -1, $trail = NULL) {
  $output = '';
  $index = 0;
  $count = 0;
...
function fusion_widget_preprocess_search_block_form(&$variables) {
  $search_value = 'Search';
...
function fusion_widget_preprocess_views_view(&$variables) {
