<?php
function fusion_wcm_preprocess_node(&$vars, $hook) {
  if ($vars['teaser']) {
    $vars['template_files'][] = "node-{$vars['node']->type}-teaser";
  }

  $function = "fusion_wcm_preprocess_node_{$vars['node']->type}";
  if (function_exists($function)) {
    $function($vars);
  }
}

function fusion_wcm_preprocess_node_product(&$vars) {
  $node = $vars['node'];
  if (!$vars['teaser']) {
...
function fusion_wcm_nice_menu_build($menu, $depth = -1, $trail = NULL) {
  $output = '';
  $index = 0;
  $count = 0;
...
function fusion_wcm_preprocess_search_block_form(&$variables) {
  $search_value = 'Search';
